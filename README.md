...
└── tasks.json              # VS Code tasks (optional): run backend and security tests easily
...
```
docker build -t <your-dockerhub-username>/safe-code-executor:latest .
```
Example:
```
...
```
docker push <your-dockerhub-username>/safe-code-executor:latest
```
Example push:
```
